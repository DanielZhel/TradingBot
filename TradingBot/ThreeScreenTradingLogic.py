...
def get_short_positions_macdema(inst_data2,
    ...
    for j in range (len(macd_values[0][0])-1):
        ...
        i=2   
        while i < len(stoch_values[0]):
            # Проверка нахождения цены в интервале от ЕМА до ЕМА-%
            if (ema_min<=inst_data2['open'][i]<=ema_values[0][j] and  inst_data2['open'][i]> inst_data2['close'][i] and  ema_min<=inst_data2['open'][i-1]<=ema_values[0][j] and ema_min<=inst_data2['open'][i-2]<=ema_values[0][j]):
                # Условие для МАКД
                if ( macd_values[0][0][j]<0 and macd_values[0][0][j]<macd_values[0][0][j-1] and macd_values[0][1][j]<macd_values[0][1][j-1]):  
                    # Условие для проверки присутствия стохаста в интервале МАКД
                    if(macd_values[1][j]<=stoch_values[2][i]<=macd_values[1][j+1]):
                        # Условие для стохаста
                        if (stoch_values[0][i-1] > high_stoch_level and stoch_values[0][i] < stoch_values[1][i] and stoch_values[0][i-1]>stoch_values[1][i-1]):
                            fast = float(stoch_values[0][i])
                            slow = float(stoch_values[1][i])
